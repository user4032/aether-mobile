...
class _ContactsScreenState extends State<ContactsScreen> {
  ...
  Future<void> _updateAvatar() async {
    ...
    if (image != null) {
      // Кропер тільки для аватарки в налаштуваннях
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _startRecording() async {
    try {
      if (await _audioRecorder.hasPermission()) {
       ...
       // ФІКС iOS: Використовуємо AAC, який є рідним для iOS/Safari
  ...
  // ФІКС 2: Ідеальне фото без кропера для чату
  ...
  // ФІКС 4: Анти-лаг кешування фото (cacheWidth)
  ...
  // ФІКС 1: Парсер посилань (URL Launcher)
