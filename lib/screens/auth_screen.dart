...
class _AuthScreenState extends State<AuthScreen> {
  ...
  void _login() async {
    ...
    io.Socket s = io.io(serverUrl, <String, dynamic>{
  ...socketOptions(),
  'forceNew': true,
});
...
connectionTimeout = Timer(const Duration(seconds: 60), () {
  if (!isConnected && mounted) {
    s.dispose();
    setState(() => isLoading = false);
    _showSnack(t('Помилка підключення', 'Connection timeout'), isError: true);
  }
});
...
s.onConnectError((error) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SizedBox.expand( // <-- ДОДАНО: Розтягуємо фон на весь екран
        child: LiquidBackground(
          child: SafeArea(
            child: Center( // <-- ДОДАНО: Центруємо по вертикалі
