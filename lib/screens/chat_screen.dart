...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _connect() {
    ...
    socket.onDisconnect((_) {
      debugPrint('Відключено від сервера...');
      if (mounted) setState(() => _isPartnerOnline = false);
    });

    socket.onReconnect((_) {
      debugPrint('З\'єднання відновлено!');
      socket.emit('set_active', widget.userName);
      if (!widget.partnerPublicKey.startsWith('GROUP_')) {
        socket.emitWithAck('check_presence', widget.partnerName, ack: (dynamic data) { 
          if (mounted) setState(() => _isPartnerOnline = data['isOnline']); 
        });
      }
      String historyPartner = widget.partnerPublicKey.startsWith('GROUP_') ? widget.partnerPublicKey : widget.partnerName;
                              ...
                              decoration: BoxDecoration(
                                color: isSearchMatch
                                    ...
                                    : (isMsgEphemeral 
                                        ...
                                        : (isMe ? const Color(0xFF2B5278) : const Color(0xFF212121))),
                                border: Border.all(
                                  color: isSearchMatch 
